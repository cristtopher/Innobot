...
def sendwhatapp(User, Message):
    password = "xx"                      #Password dada al registrar el numero.
    password = base64.b64decode(bytes(password.encode('utf-8')))   #Codificacion de Password para envio a los servidores de whatsApp.
    username = 'xx'                                       #Numero de telefono para el inicio de secion.
    keepAlive= False          
    Text = "%s -> %s" %(str(User), str(Message))                            #Conexion persistente con el servidor.
    whats = WhatsappEchoClient("xx", Text, keepAlive)     #Inicia el cliente para el envio de mensajes por WhatsApp.
    whats2 = WhatsappEchoClient("xx", Text, keepAlive)     #Inicia el cliente para el envio de mensajes por WhatsApp.
